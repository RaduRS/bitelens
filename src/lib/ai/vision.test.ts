...
describe('responseToProduct', () => {
  ...
  it('reclassifies whole_food + honey as dessert (concentrated sweetener)', () => {
    const p = responseToProduct({
      ...
      processing: 1,
      flaggedIngredients: [],
      nutrition: { servingGrams: 21, kcal: 64, protein: 0, carbs: 17, sugar: 17, fat: 0, satFat: 0, fiber: 0, sodium: 1 },
    });
  ...
  it('clamps absurd nutrition values from AI hallucinations', () => {
    const p = responseToProduct({
      ...sample,
      nutrition: { servingGrams: 0, kcal: -100, protein: 9999, carbs: 3, sugar: NaN, fat: 22, satFat: 4, fiber: 9, sodium: -50 },
    });
